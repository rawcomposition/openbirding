...
packsRoute.get("/", async (c) => {
    try {
        ...
        return c.json(packs.map(({ longName, centerLat, centerLng, ...pack }) => ({
            ...pack,
            name: longName,
            lat: centerLat,
            lng: centerLng,
    ...
    try {
        ...
        hotspots.forEach((hotspot) => {
            if (hotspot.countryCode)
                regionCodes.add(hotspot.countryCode);
            if (hotspot.subnational1Code)
                regionCodes.add(hotspot.subnational1Code);
            if (hotspot.subnational2Code)
                regionCodes.add(hotspot.subnational2Code);
        });
