...
const getParentRegions = (regionId, regionMap) => {
...
const generateLongName = (regionName, parents) => {
    const parentNames = parents.map((p) => p.name);
    return [regionName, ...parentNames].join(", ");
};
const hasChildren = (regionId, allRegionIds) => {
...
const main = async () => {
    try {
        ...
        for (let i = 0; i < regions.length; i += batchSize) {
            ...
            console.log(`Processed ${processed}/${regions.length} regions (${Math.round((processed / regions.length) * 100)}%)`);
        }
